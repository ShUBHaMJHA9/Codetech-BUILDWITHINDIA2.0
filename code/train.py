...
if __name__ == '__main__':
    ...
    # -------------------------- Load Model --------------------------
    model = YOLO(os.path.join(this_dir, "yolov8s.pt"))  # use yolov8m.pt for better accuracy
    ...
    results = model.train(
        ...
        weight_decay=0.0005,           # Important for regularization
        warmup_epochs=3,               # Helps prevent overfitting early
        mosaic=args.mosaic,            # Mosaic
        mixup=0.2,                     # Helps generalization
        copy_paste=0.1,                # Augment object presence
        hsv_h=0.015,                   # HSV aug: H
        hsv_s=0.7,                     # HSV aug: S
        hsv_v=0.4,                     # HSV aug: V
